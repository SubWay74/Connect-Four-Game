    ...
    function checkBoard() {
        for (i = 0; i < winningArrays.length; i++) {
            let squarOne = gameBoard[winningArrays[i][0]]
            let squartwo = gameBoard[winningArrays[i][1]]
            let squarThree = gameBoard[winningArrays[i][2]]
            let squarFour = gameBoard[winningArrays[i][3]]
            if (
                squarOne.classList.contains("red") &&
                squartwo.classList.contains("red") &&
                squarThree.classList.contains("red") &&
                squarFour.classList.contains("red") 
            ){
                resutlts.innerHTML = "Player One Wins!!"
                ...
                squartwo.classList.contains("yellow") &&
                squarThree.classList.contains("yellow") &&
                squarFour.classList.contains("yellow") 
            ){
                resutlts.innerHTML = "Player Two Wins!!"
        ...
        gameBoard.forEach(e => e.addEventListener("click", (el) => {
            ...
            let arr = Array.from(gameBoard).indexOf(el.target)
            if (gameOver) {
                return "";
            } else {
                ...
                userTurn = !userTurn
                playerTurn = userTurn ? "one" : "two"
                playerTurnEl.innerHTML = userTurn ? "one" : "two"
                checkBoard()
            }
            }
        }
